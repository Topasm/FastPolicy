...
class CombinedPolicy(nn.Module):
    ...
    def __init__(self, diffusion_model: MyDiffusionModel, inv_dyn_model: MlpInvDynamic):
        ...
        try:
            ...
            dataset_stats = {}
            # Convert stats to tensors and move to the correct device
            ...
            # Create normalizers with specific roles
            ...
            self.normalize_invdyn_state = Normalize(
                {"observation.state": self.config.robot_state_feature},
                self.config.normalization_mapping, dataset_stats
            )
            ...
            self.normalize_invdyn_state = lambda x: x
            self.normalize_invdyn_action = lambda x: x
    ...
    def plan_future_states(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        """
        # Using context manager for torch.no_grad()
        with torch.no_grad():
